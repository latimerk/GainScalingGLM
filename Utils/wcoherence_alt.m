function [wcoh,wcs,P,coi] = wcoherence_alt(cwtx,y,varargin)
%Wavelet coherence
...
% cwty = cwtft_alt({gpuArray(y),dt},'wavelet',wname,'scales',scales,'PadMode','symw');
cwty = cwtft_alt({y,dt},'wavelet',wname,'scales',scales,'PadMode','symw');
...
cfs2 = smoothCFS2(invscales.*abs(cwty.cfs).^2,scales,dt,ns);
crossCFS = cwtx.cfs.*conj(cwty.cfs);
crossCFS = smoothCFS2(invscales.*crossCFS,scales,dt,ns);
crosspec = crossCFS./(sqrt(cwtx.cfs1).*sqrt(cfs2));
wtc = abs(crossCFS).^2./(cwtx.cfs1.*cfs2);
N = size(cwtx.cfs1,2);
...
function cfs = smoothCFS2(cfs,scales,dt,ns)
N = size(cfs,2);
npad = 2.^nextpow2(N);
omega = 1:fix(npad/2);
omega = omega.*((2*pi)/npad);
...
F = exp(-0.25*(omega(:).^2 *normscales(1:size(cfs,1)).^2'));
smooth = ifft(F.*fft(cfs',npad));
cfs = smooth(1:N,:)';
...
function cfs = smoothCFS_gpu(cfs,scales,dt,ns)
N = size(cfs,2);
npad = 2.^nextpow2(N);
omega = 1:fix(npad/2);
omega = omega.*((2*pi)/npad);
omega2 = gpuArray([0., omega, -omega(fix((npad-1)/2):-1:1)].^2);

% Normalize scales by DT because we are not including DT in the
% angular frequencies here. The smoothing is done by multiplication in
% the Fourier domain
normscales2 = gpuArray(scales./dt).^2;
cfs_gpu = gpuArray(cfs);

F = exp(-0.25*(omega2(:)*normscales2(:)'));
smooth = ifft(F.*fft(cfs_gpu',npad));
cfs_gpu = smooth(1:N,:)';
% Convolve the coefficients with a moving average smoothing filter across
% scales
H = gpuArray(1/ns*ones(ns,1));
cfs_gpu = conv2(cfs_gpu,H,'same');
cfs = gather(cfs_gpu);
