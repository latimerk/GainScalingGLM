function CWTStruct = cwtft_alt(SIG,varargin)
...
%   The minimun normalized scale (scale/sampling period) should be strictly
...
if ~isequal(pad_MODE,'none')
    ...
    x = wextend_alt('1d',pad_MODE,x,2^np2-nbSamp,'r');
end
...
[psift,frequencies]  = waveft_alt(WAV,omega,scales);
...
CWTStruct = struct('cfs',gather(cwtcfs),'scales',scales, 'frequencies',frequencies,...
    'omega',omega,'meanSIG',gather(meanSIG),'dt',dt);
CWTStruct.wav = WAV;
